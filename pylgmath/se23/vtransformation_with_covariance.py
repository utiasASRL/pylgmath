...
from .vtransformation import VTransformation


class VTransformationWithCovariance(VTransformation):
  """Light weight transformation class with added covariance propagation."""

  def __init__(self,
               *,
               tran_w_cov: Optional[VTransformationWithCovariance] = None,
               covariance: Optional[np.ndarray] = None,
               init_cov_to_zero: bool = False,
               **kwargs) -> None:
    """Constructor with several options
    Args:
      tran_w_cov (Optional[VTransformationWithCovariance]): an existing instance of this class. (copy construction)
    ...
    self._covariance = np.zeros((9, 9))  # TODO make this the same shape as the underlying transformation
  ...
  def set_covariance(self, cov: np.ndarray) -> None:
    """Sets the 9x9 covariance matrix.
    Args:
      cov: the 9x9 covariance matrix
    """
    assert cov.shape[:-2] == self._C_ba.shape[:-2] and cov.shape[-2:] == (9, 9)
    self._covariance = cov
    self._covariance_set = True

  def set_zero_covariance(self) -> None:
    """Sets the covariance to zero."""
    self._covariance = np.zeros_like(*self._C_ba.shape[:-2], 9, 9)
    self._covariance_set = True

  def inverse(self) -> VTransformationWithCovariance:
    """Returns the inverse of this transformation including the covariance."""
    temp = VTransformationWithCovariance(transformation=super().inverse())
